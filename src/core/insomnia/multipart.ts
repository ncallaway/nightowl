...
import { RequestBodyParameter } from "../insomniaTypes";
...
export const buildMultipart = async (params: RequestBodyParameter[]) => {
  return new Promise<Multipart>(async (resolve, reject) => {
    ...
    const filePath = path.join(owlpaths.globalTempDir(), Math.random() + ".body");
